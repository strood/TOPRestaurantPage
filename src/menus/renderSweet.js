...
const renderSweet = (menuHolder) => {
  ...
  // Build menu items
  const chocoCake = buildMenuItem('mashed potato chocolate cake with penuche icing', '9', 'seriously, a rich chocolate cake,'
    + ' with walnuts, and a brown sugar fudge icing.');
  const shortBread = buildMenuItem('cheddar shortbread with spiced apple', '9', 'very traditional Scottish shortbread right off the Canada'
  + ' Corn Starch box with a very Tribe cheddar twist, served with spiced apple compote.');
  const lemon = buildMenuItem('lemon jar', '9', 'when life gives you lemons, tribe gives you lemon cheesecake... and berries');
  const gelato = buildMenuItem('tribe gelato', '9', 'we make our gelato in house. ask about our featured flavours. each is paired with an accoutrement... something perfect.');
  const chocoPot = buildMenuItem('ay caramba chocolate pot', '9', 'mexican-spiced, rich, creamy, chocolate, with fire dust to warm your cockles.');
