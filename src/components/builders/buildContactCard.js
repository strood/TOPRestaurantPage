...
const buildContactCard = (article) => {
  const tel = '+1 403-392-3046';
  const email = 'eat@tribeflatout.com';
  // Create text elements
  let contactText = document.createElement('pre');
  contactText = '           To make a reservation by phone, call:\n'
    + `                          ${tel} `
  ...
  const contactCard = buildArticleCard('- Details -', contactText, colorPicker(4));
